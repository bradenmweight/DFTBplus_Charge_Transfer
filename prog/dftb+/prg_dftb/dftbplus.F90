...
!  use emfields
  ...
  ! General variables for nonadiabatic dynamics ! Sharma added
  integer                  :: beginSurfId, endSurfId, iSurf, jSurf, nSurf
  integer                  :: loopIncrement, iNAMDMethod, iBqPropagScheme
  integer                  :: iFrag, nFrags, bindex, eindex
  integer                  :: iBeginAOId, iEndAOId 
  integer, allocatable     :: beginAOId(:), endAOId(:)
  real(dp), allocatable    :: oldEigVec(:,:), oldOverlap(:,:), oldEigVal(:)
  real(dp), allocatable    :: currentEigVec(:,:), currentOverlap(:,:), currentEigVal(:)
  real(dp), allocatable    :: adiaPop(:), vecOverlap(:,:), overlapReal(:,:)
  real(dp)                 :: BqZero, timeStep, den, sumCurrentBq 
  complex(dp), allocatable :: oldBq(:), currentBq(:), coherencePop(:,:)
  character(mc)            :: currentTimeStep, fmtAdiaPop, fmtDiaPop
  character(mc)            :: fmtDervCplng, fmtCoherence, fmtActSurfEigVal
  character(lc)            :: resultsDir
  logical                  :: tPrintEvecAndOvlp, tWriteOvlpBin, tPrintEigVal, tNAMD
  logical                  :: tWriteBqCoeff, tWriteDervCplng, tWriteMOOverlap
  logical                  :: tWriteGSMOOverlap, tPrintTDSameMOOverlap
  logical                  :: tPrintCoherence, tPhaseFix, tFindMatchSurf

  ! RK4 Bq propagation  ! Sharma added
  integer                  :: iDervCplngScheme 
  logical                  :: tGSDervCplng
  real(dp), allocatable    :: oldDervCplng(:,:), currentDervCplng(:,:)

  ! For Short-time Propagation
  real(dp), allocatable    :: oldTDMOOverlap(:,:), currentTDMOOverlap(:,:)

  ! variables for FSSH ! Sharma added
  integer                  :: nIntPolSteps, newSeed(2)
  integer                  :: nFSSHTraj, iFSSHTraj, iActiveSurf, iNextSurf
  integer, allocatable     :: nextSurf(:), activeSurf(:)
  real(dp)                 :: accumulator, dice
  real(dp)                 :: est1, est2, est3
  real(dp)                 :: totEst1, totEst2, totEst3
  real(dp), allocatable    :: estimator1(:), estimator2(:), estimator3(:)
  real(dp)                 :: totEstimator1, totEstimator2, totEstimator3
  real(dp), allocatable    :: FSSHAdiaPop(:)
  real(dp), allocatable    :: hopProbability(:,:)
  real(dp)                 :: minRandNum
  integer                  :: iFSSHMethod
  logical                  :: tReadRandomSeed, tWriteIntPol, tWriteHopProb
  character(mc)            :: fmtHopProb
  character(*), parameter  :: activeSurfEnergyOut = "./active_surface_energy.dat"

  ! Sharma added File Descriptors 
  integer :: fdNAMDInp           ! NAMD input
  integer :: fdBqZeroInp         ! Bq Zero input
  integer :: fdBqCoeffOut        ! Bq output
  integer :: fdAdiaPopOut        ! Adiabatic population output
  integer :: fdDiaPopOut         ! Diabatic population output
  integer :: fdEigVal            ! eigenval output
  integer :: fdOverlap           ! overlap output
  integer :: fdSurfEigVal        ! Only important MOs eigenval output
  integer :: fdSubotnikEst2      ! Subotnik's Chrg pop Est2 from FSSH 
  integer :: fdFSSHAdiaPopOut    ! Subotnik's Chrg pop Est2 from FSSH 
  integer :: fdSubotnikEst1      ! Subotnik's Chrg pop Est1 from FSSH 
  integer :: fdSubotnikEst3      ! Subotnik's Chrg pop Est3 from FSSH 
  integer :: fdWfnEnergy         ! Wave Function energy based on Adia pop
  integer :: fdFSSHWfnEnergy     ! Wave Function energy based on FSSH pop
  integer :: fdMOOverlap         ! Overlap b/w MOs at different times
  integer :: fdGSMOOverlap       ! Overlap b/w MOs at different times with GS
  integer :: fdDervCplng         ! Derivative Coupling
  integer :: fdIntpolEigVal      ! Interpolated EigenValues
  integer :: fdIntPolDervCplng   ! Interpolated Derivative Coupling
  integer :: fdPrintTDSameMOOverlap  ! Overlap b/w same MOs at different times
  integer :: fdCoherenceReal     ! Real part of the Coherence
  integer :: fdCoherenceImag     ! Imag part of the Coherence
  integer :: fdHopProb           ! Hopping Probabilites (g_kj) k=activeState
  integer :: fdRandNumSeq        ! Random Numbers generated 
  ...
  character(*), parameter :: namdInp = "./namd.inp"
  ...
  !----------------------------------
  ! Sharma's modification STARTs here
  !----------------------------------
  fdNAMDInp = getFileId()
  open(fdNAMDInp, file=namdInp, action="read")
    ...
    read(fdNAMDInp,*) tPhaseFix, tFindMatchSurf
    read(fdNAMDInp,*) tPrintTDSameMOOverlap
    read(fdNAMDInp,*) tPrintCoherence
    read(fdNAMDInp,*) timeStep, loopIncrement
    read(fdNAMDInp,*) beginSurfId, endSurfId
    read(fdNAMDInp,*) iBqPropagScheme 
    read(fdNAMDInp,*) iDervCplngScheme 
    read(fdNAMDInp,*) tGSDervCplng
    read(fdNAMDInp,*) tWriteDervCplng, tWriteMOOverlap, tWriteGSMOOverlap
    read(fdNAMDInp,*) nIntPolSteps, tWriteIntPol
    ...
    read(fdNAMDInp,*) iFSSHMethod
    read(fdNAMDInp,*) tReadRandomSeed
    read(fdNAMDInp,*) newSeed(1)
    read(fdNAMDInp,*) newSeed(2)
    read(fdNAMDInp,*) tWriteHopProb
    read(fdNAMDInp,*) minRandNum 
  ...
  write(fmtAdiaPop,'(a, i4, a)') '(f14.6,', nSurf+1, 'f10.6)'
  write(fmtDiaPop,'(a, i4, a)') '(f14.6,', nFrags+1, 'f10.6)'
  write(fmtDervCplng,'(a, i4, a)') '(', nSurf,'f10.6)'
  write(fmtHopProb,'(a, i4, a)') '(f14.6,', nSurf,'f10.6)'
  write(fmtCoherence,'(a, i4, a)') '(f12.6,', nSurf*nSurf,'f10.6)'
  write(fmtActSurfEigVal,'(a, i4, a)') '(f12.6,', nSurf+10,'f10.6)'

  if (tNAMD) then

    if(iNAMDMethod == 1) then
      ALLOCATE_(activeSurf,(1))
      activeSurf(1) = 1
    else if(iNAMDMethod == 2) then
      ALLOCATE_(activeSurf,(nFSSHTraj))
    end if

    fdBqZeroInp = getFileId()
    fdSubotnikEst2 = getFileId()
    fdAdiaPopOut = getFileId()
    fdSurfEigVal = getFileId()
    fdWfnEnergy = getFileId()
    open(fdSurfEigVal, file=activeSurfEnergyOut, position="rewind", status="replace")
    open(fdAdiaPopOut,file='./'//trim(resultsDir)//'/adiabatic_pop.txt',action='readwrite')
    open(fdSubotnikEst2,file='./'//trim(resultsDir)//'/estimator2.txt',action='readwrite')
    open(fdWfnEnergy,file='./'//trim(resultsDir)//'/Wfn_energy_Adia_pop.txt',action='readwrite')

    if (tPrintEigVal) then
      fdEigVal = getFileId()
    end if

    if (tWriteIntPol) then
      fdIntpolEigVal = getFileId()
      fdIntPolDervCplng = getFileId()
      open(fdIntpolEigVal,file='./intPolEigVal.txt',action='readwrite')
      open(fdIntPolDervCplng,file='./intPolDervCplng.txt',action='readwrite')
    end if

    if(tPhaseFix) then
      if (tPrintTDSameMOOverlap) then
        fdPrintTDSameMOOverlap = getFileId()
        open(fdPrintTDSameMOOverlap,file='./sameMO_Ovlp_Across_timesteps.txt',action='readwrite')
      end if
    end if

    if (tPrintCoherence) then
      fdCoherenceReal = getFileId()
      open(fdCoherenceReal,file='./real_coherence.txt',action='readwrite')
      !open(fdCoherenceImag,file='./imag_coherence.txt',action='readwrite')
    end if

    if (tPrintEvecAndOvlp) then
      fdOverlap = getFileId()
    end if

    if (tWriteBqCoeff) then
      fdBqCoeffOut = getFileId()
    end if

    if (tWriteMOOverlap) then
      fdMOOverlap = getFileId()
      open(fdMOOverlap,file='./'//trim(resultsDir)//'/MOOverlap.txt',action='readwrite')
    end if

    if (tWriteGSMOOverlap) then
      fdGSMOOverlap = getFileId()
      open(fdGSMOOverlap,file='./'//trim(resultsDir)//'/GSMOOverlap.txt',action='readwrite')
    end if

    if (tWriteDervCplng) then
      fdDervCplng = getFileId()
      open(fdDervCplng,file='./'//trim(resultsDir)//'/dervCplng.txt',action='readwrite')
    end if

    if (iNAMDMethod == 2) then ! 2 = FSSH
      fdFSSHAdiaPopOut = getFileId()
      fdSubotnikEst1 = getFileId()
      fdSubotnikEst3 = getFileId()
      fdFSSHWfnEnergy = getFileId()
      open(fdFSSHAdiaPopOut,file='./'//trim(resultsDir)//'/FSSH_adiabatic_pop.txt',action='readwrite')
      open(fdSubotnikEst1,file='./'//trim(resultsDir)//'/estimator1.txt',action='readwrite')
      open(fdSubotnikEst3,file='./'//trim(resultsDir)//'/estimator3.txt',action='readwrite')
      open(fdFSSHWfnEnergy,file='./'//trim(resultsDir)//'/Wfn_energy_FSSH_pop.txt',action='readwrite')

      if (tReadRandomSeed) then
        call random_seed(put=newSeed) ! put seed to the function
        call random_seed(get=newSeed) ! put seed to the function
        write(*,*) "newSeed=", newSeed
      else 
        call random_seed()
        call random_seed(get=newSeed) ! put seed to the function
        write(*,*) "newSeed=", newSeed
      ...
      if (tWriteHopProb) then
        fdHopProb = getFileId()
        open(fdHopProb,file='./'//trim(resultsDir)//'/hop_prob_g_kj.txt',action='readwrite')
      end if 
   
      if(nFSSHTraj == 1) then
        fdRandNumSeq = getFileId()
        open(fdRandNumSeq,file='./'//trim(resultsDir)//'/generated_random_numbers.txt',action='readwrite')
      end if
  ...
  !----------------------------------
  ! Sharma's modification ENDs here
  !----------------------------------
  ...
  ! Sharma's modification STARTs here
  ! ---------------------------------
  ALLOCATE_(overlapReal, (sqrHamSize, sqrHamSize))
  ALLOCATE_(oldEigVal, (nSurf))
  ALLOCATE_(oldEigVec, (sqrHamSize, nSurf))
  ALLOCATE_(oldOverlap, (sqrHamSize, sqrHamSize))
  ALLOCATE_(oldBq, (nSurf))
  ALLOCATE_(adiaPop, (nSurf))
  ALLOCATE_(FSSHAdiaPop, (nSurf))
  ALLOCATE_(estimator1, (nFrags))
  ALLOCATE_(estimator2, (nFrags))
  ALLOCATE_(estimator3, (nFrags))
  ALLOCATE_(currentEigVal, (nSurf))
  ALLOCATE_(currentEigVec, (sqrHamSize, nSurf))
  ALLOCATE_(currentOverlap, (sqrHamSize, sqrHamSize))
  ALLOCATE_(currentBq, (nSurf))
  ALLOCATE_(vecOverlap, (nSurf,nSurf))
  ALLOCATE_(oldDervCplng, (nSurf,nSurf))
  ALLOCATE_(currentDervCplng, (nSurf,nSurf))
  ALLOCATE_(oldTDMOOverlap, (nSurf,nSurf))
  ALLOCATE_(currentTDMOOverlap, (nSurf,nSurf))
  ALLOCATE_(coherencePop, (nSurf,nSurf))
  ALLOCATE_(hopProbability, (nSurf,nSurf))
  ALLOCATE_(nextSurf,(nFSSHTraj))
  ! ---------------------------------
  ! Sharma's modification ENDs here
  ! ---------------------------------
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  if (tNAMD) then
    if (tWriteBqCoeff) then
      call execute_command_line('mkdir -p B_q_dir/')
    end if
    if (tPrintEvecAndOvlp) then
      call execute_command_line('mkdir -p eigvecs_dir/')
      call execute_command_line('mkdir -p overlap_dir/')
    end if
    if (tPrintEigVal) then
      call execute_command_line('mkdir -p eigvals_dir/')
    end if
  end if
  ! ---------------------------------
  ! Sharma's modification ENDs here
  ...
  lpGeomOpt: do while (iGeoStep <= nGeoSteps)
    ...
    ! Print ovlp for every geometry step irrespective of tWriteHS is T or F
    ! Overlap should be printed here as diagonalization can introduce errors
    ! in the stored overlap due to Cholskey factorization!
    ! ---------------
    ! Sharma extra print STARTs here
    ! ---------------
    if (tPrintEvecAndOvlp) then
      if (tImHam) then
        call writeHS(tWriteHS, tWriteRealHS, ham, over, &
          & neighborList%iNeighbor, nNeighbor, iAtomStart, iPair, &
          & img2CentCell, kPoint, iCellVec, cellVec, i2c(iGeoStep),&
          & fdOverlap, tWriteOvlpBin, iHam=iHam)
      else
        call writeHS(tWriteHS, tWriteRealHS, ham, over, &
          & neighborList%iNeighbor, nNeighbor, iAtomStart, iPair, &
          & img2CentCell, kPoint, iCellVec, cellVec, i2c(iGeoStep), &
          & fdOverlap, tWriteOvlpBin)
      end if
      write (*, "(A)") "Overlap written, before entering SCC loop."
    end if
    ...
    if (tNAMD) then
      ...
      elseif (iGeoStep > 0) then
        oldOverlap = currentOverlap
        currentOverlap = overlapReal
      end if
    end if
    ! ---------------
    ! Sharma extra print ENDs here
    ! ---------------
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    ! ---------------------------------
    if (tPrintEigVal) then
      close(fdEigVal) ! To be safeside
      write (tmpStr, "(A,A,A)") "./eigvals_dir/eigvals_", trim(i2c(iGeoStep)), ".dat"
      open(fdEigVal, file=tmpStr, position="rewind", status="replace")

      lpEigValPrint: do iSpin = 1, nSpinHams
        do iEgy = 1, sqrHamSize
          write(fdEigVal, formatEigen) &
              &(Hartree__eV*eigen(iEgy,ii,iSpin),ii=1,nKPoint)
        end do
        write(fdEigVal,*)'Above Eigenvalues /eV'

        write(fdEigVal,*)'Below Eigenvalues /H'
        do iEgy = 1, sqrHamSize
          write(fdEigVal, formatEigen) (eigen(iEgy,ii,iSpin),ii=1,nKPoint)
        end do
      end do lpEigValPrint

      close(fdEigVal)
    end if

    ! Everything related to nonadiabatic dynamics 
    if (tNAMD) then
      if (iGeoStep == 0) then

        open(fdBqZeroInp,file='./'//trim(resultsDir)//'/B_q_zero.txt',action='read')
          ! Reading the unnecessary elements of BqZero
          do iSurf=1,beginSurfId -1
            read(fdBqZeroInp,*) BqZero  ! real
          end do
          ! Read the necessary elements of BqZero
          do iSurf=1,nSurf ! Actually, reading q=beginSurfId,endSurfId
            read(fdBqZeroInp,*) BqZero  ! real
            currentBq(iSurf) = cmplx(BqZero)
          end do
        close(fdBqZeroInp)

        if(tWriteBqCoeff) then
          currentTimeStep = i2c(iGeoStep)
          open(fdBqCoeffOut,file='./'//trim(resultsDir)//'/B_q_dir/B_q_'//trim(currentTimeStep)//'.dat')
            do iSurf = 1, nSurf 
              write(fdBqCoeffOut,'(f15.10,f15.10)') currentBq(iSurf)
            end do
          close(fdBqCoeffOut)
        end if
        ...
        sumCurrentBq = 0.0d0
        do iSurf = 1, nSurf
          sumCurrentBq = sumCurrentBq + currentBq(iSurf)*conjg(currentBq(iSurf))
        end do
        currentBq = currentBq*(1.0d0/sumCurrentBq)
  
        do iSurf = 1, nSurf
          adiaPop(iSurf) = conjg(currentBq(iSurf))*(currentBq(iSurf))
        end do
  
        write(fdAdiaPopOut,fmtAdiaPop) iGeoStep*timeStep,&
          & (adiaPop(iSurf),iSurf=1,nSurf), sum(adiaPop(1:nSurf))

        if(tPrintCoherence) then
          do iSurf = 1, nSurf
            do jSurf = 1, nSurf
              coherencePop(jSurf,iSurf) = conjg(currentBq(jSurf))*(currentBq(iSurf))
            end do
          end do
          write(fdCoherenceReal,fmtCoherence) iGeoStep*timeStep, real(coherencePop)
          !write(fdCoherenceImag,fmtCoherence) iGeoStep*timeStep, aimag(coherencePop)
        end if

        !------------------------------------------------
        ! Pick active states for each FSSH trajectory
        !------------------------------------------------
        ! We initialized activeSurf(iFSSHTraj) = MAXLOC(adiaPop,1) because sometimes
        ! "accmumulator" cannot be greater than "dice" in pickBq subroutine and
        ! thus the routine exits without assigning any active surface. If we 
        ! initialize activeSurf(iFSSHTraj) = 0 then this will lead to an error
        ! as the activeSurf should have a value between 1-10.
        !------------------------------------------------
        if (iNAMDMethod == 2) then ! 2 = FSSH
          activeSurf = MAXLOC(adiaPop,1)
          do iFSSHTraj = 1, nFSSHTraj
            call pickActiveSurf(nSurf, beginSurfId, endSurfId, adiaPop,&
              & accumulator, dice, activeSurf(iFSSHTraj))
            write(*,'(a,i7,a,2f14.6,i4)') "accum, dice, picked Bq for", iFSSHTraj,&
              & "th SH traj:", accumulator, dice, activeSurf(iFSSHTraj)
            nextSurf(iFSSHTraj) = activeSurf(iFSSHTraj)
          end do
        end if

        ! Eigen Values and Vectors
        do iSurf = 1,nSurf
          currentEigVal(iSurf)= Hartree__eV*eigen(beginSurfID+iSurf-1,1,1)
          currentEigVec(:,iSurf)= HSqrReal(:,beginSurfID+iSurf-1,1) 
        end do

        if(nFSSHTraj > 999) then
          write(fdSurfEigVal,fmtActSurfEigVal) iGeoStep*timeStep,&
            & (currentEigVal(iSurf),iSurf=1,nSurf),& 
            & currentEigVal(activeSurf(1)), currentEigVal(activeSurf(100)),&
            & currentEigVal(activeSurf(200)), currentEigVal(activeSurf(300)),&
            & currentEigVal(activeSurf(400)), currentEigVal(activeSurf(500)),&
            & currentEigVal(activeSurf(600)), currentEigVal(activeSurf(700)),&
            & currentEigVal(activeSurf(800)), currentEigVal(activeSurf(900))
        else
          write(fdSurfEigVal,fmtAdiaPop) iGeoStep*timeStep,&
            & (currentEigVal(iSurf),iSurf=1,nSurf), currentEigVal(activeSurf(1))
        end if

        call subotnikEstimators(sqrHamSize, nSurf, currentBq, currentEigVec,&
          & currentOverlap, beginAOId, endAOId, estimator2, totEstimator2)

        write(fdSubotnikEst2, fmtDiaPop) iGeoStep*timeStep,&
          & (estimator2(iFrag),iFrag=1,nFrags), totEstimator2

        write(fdWfnEnergy,fmtAdiaPop) iGeoStep*timeStep,&
          & (adiaPop(iSurf)*currentEigVal(iSurf),iSurf=1,nSurf),&
          & sum(adiaPop(:)*currentEigVal(:))

        ! Not included in the above if condition as Eigval is not defined yet.
        ! Remember that correct activeSurf is required for printing its energy.
        ! So, we can't bring even the above if condition to here.
        if (iNAMDMethod == 2) then ! 2 = FSSH
          FSSHAdiaPop = 0.0d0
          do iFSSHTraj = 1, nFSSHTraj
            FSSHAdiaPop(activeSurf(iFSSHTraj)) = FSSHAdiaPop(activeSurf(iFSSHTraj)) + 1
            activeSurf(iFSSHTraj) = nextSurf(iFSSHTraj)
          end do
          FSSHAdiaPop(:) = FSSHAdiaPop(:)/nFSSHtraj

          write(fdFSSHAdiaPopOut,fmtAdiaPop) iGeoStep*timeStep,&
            & (FSSHAdiaPop(iSurf),iSurf=1,nSurf), SUM(FSSHAdiaPop(1:nSurf))

          call subotnikEstimators(sqrHamSize, nSurf, currentBq, currentEigVec,&
            & currentOverlap, beginAOId, endAOId, estimator2, totEstimator2,&
            & FSSHAdiaPop, estimator1, totEstimator1, estimator3, totEstimator3)

          write(fdSubotnikEst1, fmtDiaPop) iGeoStep*timeStep,&
            & (estimator1(iFrag),iFrag=1,nFrags), totEstimator1
          write(fdSubotnikEst3, fmtDiaPop) iGeoStep*timeStep,&
            & (estimator3(iFrag),iFrag=1,nFrags), totEstimator3

          write(fdFSSHWfnEnergy,fmtAdiaPop) iGeoStep*timeStep,&
            & (FSSHAdiaPop(iSurf)*currentEigVal(iSurf),iSurf=1,nSurf),&
            & sum(FSSHAdiaPop(:)*currentEigVal(:))
        end if

      elseif (iGeoStep == 1) then

        oldEigval = currentEigVal
        oldEigVec = currentEigVec
        oldBq = currentBq
        ...
        call oldAndCurrentEigVecOverlap(sqrHamSize, nSurf, oldEigVec,&
          & oldOverlap, currentEigVec, currentOverlap, currentTDMOOverlap)

        if(tPhaseFix) then
          if(tPrintTDSameMOOverlap) then
            call phaseFix(nSurf, currentEigVec, currentTDMOOverlap,&
              & tFindMatchSurf, tPrintTDSameMOOverlap, fdPrintTDSameMOOverlap)
          else
            call phaseFix(nSurf, currentEigVec, currentTDMOOverlap,&
              & tFindMatchSurf, tPrintTDSameMOOverlap)
          end if
        end if
        ...
        call derivativeCoupling(nSurf, timeStep, currentTDMOOverlap,&
          & iDervCplngScheme, tGSDervCplng, currentDervCplng)

        if(tWriteMOOverlap) then
          write(fdMOOverlap,fmtCoherence) iGeoStep*timeStep, currentTDMOOverlap
        end if

        if(tWriteDervCplng) then
          write(fdDervCplng,fmtCoherence) iGeoStep*timeStep, currentDervCplng
        end if

        if (iNAMDMethod == 1) then ! 1 = Ehrenfest
          call modifiedGramSchmidt(nSurf, currentTDMOOverlap) 
          call shortTimeBqPropagation(timeStep, nSurf, oldBq, oldEigVal,&
            & currentEigVal, currentTDMOOverlap, currentBq)

          if(tWriteGSMOOverlap) then
            write(fdGSMOOverlap,fmtCoherence) iGeoStep*timeStep, currentTDMOOverlap
          end if

        else if (iNAMDMethod == 2) then ! 2 = FSSH

          ! shortTimeBqPropagation is CALLed irrespective of the propagation
          ! scheme, because, for RK4 propagation scheme, we need both old and
          ! current derivative couplings and to calculate both of them, we need
          ! the information from 3 time steps. 
      ...
      else if (iGeoStep > 1) then
         
        ! **********************************************************
        ! You should think on how to implement "loopIncrement" here
        ! I guess, it will be something like
        ! if (mod(iGeoStep,loopIncrement) == 0) then
        ! .....
        ! **********************************************************
        ...
        oldEigVec = currentEigVec
        oldBq = currentBq
        oldDervCplng = currentDervCplng
        oldTDMOOverlap = currentTDMOOverlap
        ...
        ! Earlier call is to fix the phase and the current call is to
        ! calculate MOoverlap after fixing the phase
        call oldAndCurrentEigVecOverlap(sqrHamSize, nSurf, oldEigVec,&
          & oldOverlap, currentEigVec, currentOverlap, currentTDMOOverlap)

        ! Derivative Coupling is kept out of propagation Schemes as it might be
        ! required for NAMD methods like FSSH even when we don't use RK4 method
        ! for Bq propagation.
        call derivativeCoupling(nSurf, timeStep, currentTDMOOverlap,&
          & iDervCplngScheme, tGSDervCplng, currentDervCplng)
        ...
        if (iNAMDMethod == 1) then ! 1 = Ehrenfest
          ...
          ! we need to call GS if we want to use the currentTDMOOverlap to be 
          ! used in the interpolation during short-time propagation scheme.  
          call modifiedGramSchmidt(nSurf, currentTDMOOverlap) 
          if(tWriteIntPol) then
            call linearInterpolation(timeStep, nIntPolSteps, nSurf,&
              & beginSurfId, endSurfId, oldBq, oldEigVal, oldTDMOOverlap, oldDervCplng,&
              & currentBq, currentEigVal, currentTDMOOverlap,currentDervCplng,&
              & iFSSHMethod, iBqPropagScheme, hopProbability, tWriteIntPol,&
              & fdIntpolEigVal, fdIntPolDervCplng)
          else
            call linearInterpolation(timeStep, nIntPolSteps, nSurf,&
              & beginSurfId, endSurfId, oldBq, oldEigVal, oldTDMOOverlap, oldDervCplng,&
              & currentBq, currentEigVal, currentTDMOOverlap,currentDervCplng,&
          ...
          if(tWriteHopProb) then
            write(fdHopProb,fmtHopProb) iGeoStep*timeStep, hopProbability(activeSurf(1),:) 
          end if
          
          if(nFSSHTraj < 25) then
            do iFSSHTraj = 1, nFSSHTraj
              call FSSHmethod(nSurf, hopProbability, activeSurf(iFSSHTraj),&
                & nextSurf(iFSSHTraj), minRandNum, fdRandNumSeq)
            end do
          else if(nFSSHTraj > 24) then
            !!$omp parallel do
              do iFSSHTraj = 1, nFSSHTraj
                call FSSHmethod(nSurf, hopProbability, activeSurf(iFSSHTraj),&
                  & nextSurf(iFSSHTraj), minRandNum)
              end do
            !!$omp end parallel do
          end if
        ...
        end if ! iNAMDMethod == 1 or 2 or etc.
      ...
      end if ! iGeoStep == 0 or not if condition 
    end if ! tNAMD if condition
    !---------------------------
    ! Sharma's modification ENDs 
    !---------------------------
        ...
        if (nSpin == 4) then
          if (tStoreEigvecs) then
            ...
            call writeEigvecs(fdEigvec, runId, nAtom, nSpin, neighborList,&
                ...
                & reshape([0.0_dp,0.0_dp,0.0_dp],(/3,1/)), HSqrCplx, SSqrCplx, i2c(iGeoStep),&
                & storeEigvecsCplx)
          else
            call writeEigvecs(fdEigvec, runId, nAtom, nSpin, neighborList,&
                & nNeighbor, cellVec, iCellVec, iAtomStart, iPair, &
          ...
          if (tStoreEigvecs) then
            call writeEigvecs(fdEigvec, runId, nAtom, nSpin, neighborList, &
                &nNeighbor, iAtomStart, iPair, img2CentCell, orb, species, &
                &speciesName, over, HSqrReal, SSqrReal, i2c(iGeoStep), storeEigvecsReal)
          else
            call writeEigvecs(fdEigvec, runId, nAtom, nSpin, neighborList, &
                &nNeighbor, iAtomStart, iPair, img2CentCell, orb, species, &
                &speciesName, over, HSqrReal, SSqrReal, i2c(iGeoStep))
          end if
        end if
      else
        if (tStoreEigvecs) then
          call writeEigvecs(fdEigvec, runId, nAtom, nSpin, neighborList, &
              ...
              &orb, species, speciesName, over, kpoint, HSqrCplx, SSqrCplx, i2c(iGeoStep), &
              &storeEigvecsCplx)
        else
          call writeEigvecs(fdEigvec, runId, nAtom, nSpin, neighborList,&
              &nNeighbor, cellVec, iCellVec, iAtomStart, iPair, img2CentCell,&
  ...
  !---------------------------------
  ! Sharma's modifcation STARTs here 
  !---------------------------------
  DEALLOCATE_(beginAOId)
  DEALLOCATE_(endAOId)
  DEALLOCATE_(oldEigVal)
  DEALLOCATE_(oldEigVec)
  DEALLOCATE_(oldOverlap)
  DEALLOCATE_(oldBq)
  DEALLOCATE_(currentEigVal)
  DEALLOCATE_(currentEigVec)
  DEALLOCATE_(currentOverlap)
  DEALLOCATE_(currentBq)
  DEALLOCATE_(nextSurf)
  DEALLOCATE_(activeSurf)
  DEALLOCATE_(adiaPop)
  DEALLOCATE_(FSSHAdiaPop)

  close(fdSurfEigVal)
  close(fdAdiaPopOut)
  close(fdSubotnikEst2)
  close(fdWfnEnergy)
  if (iNAMDMethod == 2) then ! 2 = FSSH
    close(fdFSSHAdiaPopOut)
    close(fdSubotnikEst1)
    close(fdSubotnikEst3)
    close(fdRandNumSeq)
    close(fdFSSHWfnEnergy)
  end if
  !---------------------------------
  ! Sharma's modifcation ENDs here 
  !---------------------------------
...
contains
  ...
  subroutine writeHS(tWriteHS, tWriteRealHS, ham, over, iNeighbor, &
      ...
      &cellVec, mdStepNum, fdOverlap, tWriteOvlpBin, iHam)
    ...
    character(len=*), intent(in), optional :: mdStepNum ! Sharma added
    integer, intent(in), optional :: fdOverlap ! Sharma added
    logical, intent(in), optional :: tWriteOvlpBin
    real(dp), intent(in), optional :: iHam(:,:)

    integer :: iS, nSpin
    character(lc) :: tmpStr ! Sharma added
   
    nSpin = size(ham, dim=2)

    ! Sharma's modification STARTs 
    if (present(mdStepNum)) then
      if (tRealHS) then
        if(tWriteOvlpBin) then ! If you want unformatted output file

          write (tmpStr, "(A,A,A)") "./overlap_dir/overlap_", trim(mdStepNum), ".bin" 
          call writeSparseAsSquare(tmpStr, over, iNeighbor, nNeighbor, &
              &iAtomStart, iPair, img2CentCell,fdOverlap,tWriteOvlpBin)

        else ! If you want formatted output file

          write (tmpStr, "(A,A,A)") "./overlap_dir/overlap_", trim(mdStepNum), ".dat" 
          call writeSparseAsSquare(tmpStr, over, iNeighbor, nNeighbor, &
              &iAtomStart, iPair, img2CentCell,fdOverlap,tWriteOvlpBin)
          ! call execute_command_line(" sed -i '1,5d' ./overlap_dir/overlap_" // adjustl(trim(mdStepNum)) // ".dat")
  ...
  subroutine shortTimeBqPropagation(timeStep, nSurf, oldBq, oldEigVal,&
    & currentEigVal, currentTDMOOverlap, currentBq)
    use omp_lib
    ...
    real(dp), intent(in) :: oldEigVal(:), currentEigVal(:) !! E(q,t-1), E(q,t) 
    real(dp), intent(in) :: currentTDMOOverlap(:,:)
    complex(dp), intent(in) :: oldBq(:) ! B(q,t-1) 
    complex(dp), intent(out) :: currentBq(:) ! B(q,t) 
    
    !Local variables
    integer :: iOldSurf, iNewSurf, iSurf
    real(dp), parameter :: inver_hbar = 1.0d0/0.65821189916d0 ! hbar in eV*fs
    complex(dp) :: eye

    eye=cmplx(0.0d0,1.0d0)
    ...
    if (nSurf > 24) then
      !$omp parallel do
        do iNewSurf = 1, nSurf
          currentBq(iNewSurf) = SUM((oldBq(:) * currentTDMOOverlap(:,iNewSurf) * &
                             & exp( (-eye*inver_hbar) * (oldEigVal(:)+ &
                             & currentEigVal(iNewSurf)) * (timeStep * 0.5d0))))
        end do
      !$omp end parallel do
    else
      do iNewSurf = 1, nSurf
    ...
    !================================================
    ! To check when short-time equals RK4 propagation
    ! keep the below loop after GS orthonormalization.
    !! do iNewSurf = 1, nSurf
    !!   currentTDMOOverlap(iNewSurf,iNewSurf) = 1.0d0
    !! end do
    !================================================

  end subroutine shortTimeBqPropagation

  subroutine RK4BqPropagation(timeStep, nSurf, oldBq, oldEigVal,&
    & oldDervCplng, currentBq, currentEigVal, currentDervCplng)
    
    implicit none
    !Interface variables
    integer, intent(in) :: nSurf
    real(dp), intent(in) :: timeStep
    real(dp), intent(in) :: oldEigVal(:), currentEigVal(:) 
    real(dp), intent(in) :: oldDervCplng(:,:), currentDervCplng(:,:) 
    complex(dp), intent(in) :: oldBq(:)
    complex(dp), intent(out) :: currentBq(:)
    
    !Local variables
    integer :: iOldSurf, iNewSurf, iSurf, iAO, jAO, u, v
    real(dp) :: halfEigVal(nSurf), halfDervCplng(nSurf,nSurf)
    real(dp), parameter :: inver_hbar = 1.0d0/0.65821189916d0 ! hbar in eV*fs

    complex(dp) :: k1(nSurf),k2(nSurf),k3(nSurf),k4(nSurf)
    complex(dp) :: eye, temp
    ...
    halfEigVal = 0.5d0*(oldEigVal + currentEigVal)
    halfDervCplng = 0.5d0*(oldDervCplng + currentDervCplng)
    
    do iSurf = 1, nSurf
      k1(iSurf) = -( (eye*inver_hbar) * (oldEigVal(iSurf)*oldBq(iSurf)) ) & ! - temp 
                 & -sum(oldDervCplng(iSurf,:)*oldBq(:))
    end do

    do iSurf = 1, nSurf
      k2(iSurf) = -( (eye*inver_hbar) * halfEigVal(iSurf) * &
                 & (oldBq(iSurf) + (0.5d0*timeStep*k1(iSurf))) ) &
                 & - sum(halfDervCplng(iSurf,:)*(oldBq(:) + (0.5d0*timeStep*k1(:))))
    end do

    do iSurf = 1, nSurf
      k3(iSurf) = -( (eye*inver_hbar) * halfEigVal(iSurf) * &
                 & (oldBq(iSurf) + (0.5d0*timeStep*k2(iSurf))) ) &
                 & - sum(halfDervCplng(iSurf,:)*(oldBq(:) + (0.5d0*timeStep*k2(:))))
    end do

    do iSurf = 1, nSurf
      k4(iSurf) = -( (eye*inver_hbar) * currentEigVal(iSurf) * &
                 & (oldBq(iSurf) + (timeStep*k3(iSurf))) ) &
                 & - sum(currentDervCplng(iSurf,:)*(oldBq(:) + (timeStep*k3(:))))
    end do
    ...
    do iSurf = 1, nSurf
      currentBq(iSurf) = oldBq(iSurf) + (timeStep/6.0d0)*( k1(iSurf) &
                        & + 2.0d0*(k2(iSurf) + k3(iSurf)) + k4(iSurf) )
    end do

  end subroutine RK4BqPropagation

  subroutine derivativeCoupling(nSurf, timeStep, currentTDMOOverlap,&
    & iDervCplngScheme, tGSDervCplng, currentDervCplng)
    
    implicit none
    !Interface variables
    integer, intent(in) :: nSurf
    integer, intent(in) :: iDervCplngScheme 
    logical, intent(in) :: tGSDervCplng
    real(dp), intent(in) :: timeStep
    real(dp), intent(in) :: currentTDMOOverlap(:,:)
    real(dp), intent(out) :: currentDervCplng(:,:) ! B(q,t) 
    !! integer, intent(in) :: sqrHamSize, nSurf, beginSurfId, endSurfId
    !! real(dp), intent(in) :: oldOverlap(:,:), currentOverlap(:,:)
    !! real(dp), intent(in) :: oldEigVec(:,:), currentEigVec(:,:) 
    
    !Local variables
    integer :: iOldSurf, iNewSurf, iSurf, iAO, jAO, u, v
    real(dp) :: tempMOOverlap(nSurf,nSurf)
    complex(dp) :: eye
    !! real(dp) :: tempcurrentDervCplng(nSurf,nSurf) 
    !! CHARACTER(20) :: my_fmt1

    eye=cmplx(0.0d0,1.0d0)

    !------------------------------------------------------------------------- 
    ! d_pq = (1/2*tau)*( <phi_p(t1)|phi_q(t2)> - <phi_p(t2)|phi_q(t1)> )
    !
    ! If we define b_pq = <phi_p(t1)|phi_q(t2)>,
    ! then <phi_p(t2)|phi_q(t1)> =  (b_qp)^(dagger) 
    !
    ! For real phi_p, (b_qp)^(dagger) = b_qp. As, currently, phi_p's are REAL,
    ! we have, <phi_p(t2)|phi_q(t1)> = <phi_q(t1)|phi_p(t2)>.
    !
    ! For complex phi_p, (b_qp)^(dagger) = CONJG(b_qp)
    !------------------------------------------------------------------------- 
    
    tempMOOverlap = currentTDMOOverlap
    if (tGSDervCplng) then
      call modifiedGramSchmidt(nSurf, tempMOOverlap) 
    end if
    ! tempMOOverlap is required as currentTDMOOverlap is intent in for this routine
    ! and hence, you cannot modify it by calling GS orthonormalization.

    select case(iDervCplngScheme)
      case(1) ! 1 for Hammes-Schiffer Tully way
        do iOldSurf = 1, nSurf
          currentDervCplng(iOldSurf,:) = (0.5d0/timeStep)* (tempMOOverlap(iOldSurf,:)&
                                       & - tempMOOverlap(:,iOldSurf))
        end do
  
        ! To ensure d_pp is zero, numerically
        !do iSurf = 1, nSurf
        !  currentDervCplng(iSurf,iSurf) = 0.0d0 
        !end do
  
      case(2) ! 2 for Frank's way 
        call modifiedGramSchmidt(nSurf, tempMOOverlap)
        do iOldSurf = 1, nSurf
          currentDervCplng(iOldSurf,:) = (1.0d0/timeStep)* tempMOOverlap(iOldSurf,:)
        end do

      case default ! HST way is the default
        do iOldSurf = 1, nSurf
          currentDervCplng(iOldSurf,:) = (0.5d0/timeStep)* (tempMOOverlap(iOldSurf,:)&
                                       & - tempMOOverlap(:,iOldSurf))
        end do
  ...
  !! tempcurrentDervCplng = 0.0d0
  !! do iOldSurf = 1, nSurf
  !!  do iNewSurf = 1, nSurf
  !!   do iAO = 1, sqrHamSize
  !!    do jAO = 1, sqrHamSize
  !!     tempcurrentDervCplng(iOldSurf,iNewSurf) = tempcurrentDervCplng(iOldSurf,iNewSurf) + (0.5d0*(1.0d0/timeStep)) * &
  !!                           & (oldEigVec(iAO,iOldSurf)*currentEigVec(jAO,iNewSurf)*oldOverlap(iAO,jAO) -  &
  !!                           & currentEigVec(iAO,iOldSurf)*oldEigVec(jAO,iNewSurf)*oldOverlap(iAO,jAO) )
  !!    end do
  !!   end do
  !!  end do
  !! end do

  end subroutine derivativeCoupling

  subroutine oldAndCurrentEigVecOverlap(sqrHamSize, nSurf, oldEigVec,&
    & oldOverlap, currentEigVec, currentOverlap, currentTDMOOverlap)
    use omp_lib
    implicit none
    
    !Interface variables
    integer, intent(in) :: sqrHamSize, nSurf
    real(dp), intent(in) :: oldOverlap(:,:), currentOverlap(:,:)
    real(dp), intent(in) :: oldEigVec(:,:), currentEigVec(:,:) 
    real(dp), intent(out) :: currentTDMOOverlap(:,:) ! B(q,t) 
    
    !Local variables
    integer :: iOldSurf, iNewSurf, iSurf, iAO, jAO, u, v
    real(dp) :: S(sqrHamSize,sqrHamSize)
    real(dp) :: temp1(sqrHamSize,nSurf)
    CHARACTER(20) :: my_fmt1

    !S = (currentOverlap + oldOverlap)*0.5d0
    !S = currentOverlap 
    S = oldOverlap

    !===========================================
    ! currentTDMOOverlap(p,q) = <phi_p(t1)|phi_q(t2)>
    !===========================================

    do iNewSurf = 1, nSurf
      !$omp parallel do
        do iAO= 1, sqrHamSize
          temp1(iAO,iNewSurf) =  SUM(currentEigVec(:,iNewSurf)*S(:,iAO))
        end do
      !$omp end parallel do
    end do
    ...
    if (nSurf > 24) then
      do iNewSurf = 1, nSurf
        !$omp parallel do
          do iOldSurf = 1, nSurf
            currentTDMOOverlap(iOldSurf,iNewSurf) =  SUM(oldEigVec(:,iOldSurf)*temp1(:,iNewSurf))
          end do
        !$omp end parallel do
      end do
    else
    ...
    !! write(my_fmt1,'(a, i4, a)') '(', nSurf,'f12.6)'
    !! do iSurf = 1, nSurf
    !!   write(95000,my_fmt1) currentTDMOOverlap(:,iSurf)
    !! end do
    !! write(95000,*)

  end subroutine oldAndCurrentEigVecOverlap

  subroutine modifiedGramSchmidt(nSurf,vecOverlap)
    implicit none
    !Interface variables
    integer, intent(in) :: nSurf
    real(dp), intent(inout) :: vecOverlap(:,:)
    !Local variables
    integer :: p,q,r,i,j
    real(dp) :: V(nSurf,nSurf),U(nSurf,nSurf)

    !------------------------------------------------------------
    ! For our case, each |r(t2)> corresponds to 10 vecOverlap
    ! elements, where p runs over all the nSurf at time t1.
    !------------------------------------------------------------
    do r=1,nSurf
      V(:,r)=vecOverlap(:,r)
    end do
    ...
    U = 0.0d0
    U(:,1) = V(:,1)/sqrt(abs(dot_product(V(:,1),V(:,1))))
   
    do i = 2,nSurf
      U(:,i) = V(:,i)
      do j = 1,i-1
         U(:,i) = U(:,i) - ( U(:,j) * (dot_product(U(:,i),U(:,j))) / (dot_product(U(:,j),U(:,j))) )
      end do
      U(:,i) = U(:,i)/sqrt(abs(dot_product(U(:,i),U(:,i))))
    end do

    do r=1,nSurf
     vecOverlap(:,r)= U(:,r)
    end do
  end subroutine modifiedGramSchmidt

  subroutine MOPhaseFix(nSurf, oldTDMOOverlap, currentTDMOOverlap,&
    & tFindMatchSurf, tPrintTDSameMOOverlap, fdPrintTDSameMOOverlap)
    use omp_lib
    implicit none
    !Interface variables
    logical, intent(in) :: tPrintTDSameMOOverlap
    logical, intent(in) :: tFindMatchSurf
    integer, intent(in) :: nSurf
    integer, intent(in), optional :: fdPrintTDSameMOOverlap
    real(dp), intent(in) :: oldTDMOOverlap(:,:)
    real(dp), intent(inout) :: currentTDMOOverlap(:,:) 
    
    !Local variables
    integer :: iNewSurf, iAO, jAO
    real(dp) :: modTheta(nSurf), theta(nSurf), temp1(sqrHamSize,nSurf)
    real(dp) :: timeDepMOOverlap(nSurf,nSurf)
    real(dp) :: oldOverlap(nSurf,nSurf), currentOverlap(nSurf,nSurf)
    real(dp) :: maxOvlpValue(nSurf) 
    integer :: maxOvlpSurf(nSurf) 
    real(dp), parameter :: maxTol = 0.707
    character(20) :: my_fmt4
    logical :: match(nSurf)

    write(my_fmt4,'(a, i4, a)') '(', nSurf, 'f10.6)'
    match = .false.
 
    ! Defining Normalized overlaps between MOs at different time steps
    oldOverlap = 0.0d0
    do iSurf=1,nSurf
      oldOverlap(iSurf,iSurf) = 1.0d0
    end do
    currentOverlap = oldOverlap

    !===========================================
    ! timeDepMOOverlap(p,q) = <phi_p(t1)|phi_q(t2)>
    !===========================================
    call oldAndCurrentEigVecOverlap(nSurf, nSurf, oldTDMOOverlap,&
      & oldOverlap, currentTDMOOverlap, currentOverlap, timeDepMOOverlap)
    
    ! "maxOvlpSurf" just gives the number of oldSurf which has maximum Overlap
    ! with the current Eigenvector (iNewSurf). "maxOvlpValue" gives its value
    ! "theta" not equals "maxOvlpSurf" because "maxOvlpSurf" is the absolute
    ! value

    if(tFindMatchSurf) then ! Fix the phase using Matched state's Phase
      do iNewSurf = 1, nSurf
        maxOvlpValue(iNewSurf) = maxval(abs(timeDepMOOverlap(:,iNewSurf)),dim=1)
        maxOvlpSurf(iNewSurf) = maxloc(abs(timeDepMOOverlap(:,iNewSurf)),dim=1)
        if (maxOvlpValue(iNewSurf) > maxTol) then
          theta(iNewSurf) = timeDepMOOverlap(maxOvlpSurf(iNewSurf),iNewSurf)
          match(iNewSurf) = .true.
        else ! Fix the phase using same state's Phase
          theta(iNewSurf) = timeDepMOOverlap(iNewSurf,iNewSurf) 
        end if
      enddo
    else ! Fix the phase using same state's Phase
      do iNewSurf = 1, nSurf
    ...
    if(tPrintTDSameMOOverlap) then
      write(fdPrintTDSameMOOverlap,my_fmt4) theta
      if(tFindMatchSurf) then ! Fix the phase using Matched state's Phase
        write(fdPrintTDSameMOOverlap,*) maxOvlpSurf
        write(fdPrintTDSameMOOverlap,*) match
      end if
    end if

    modTheta=ABS(theta)

    theta=theta/modTheta

    do iNewSurf = 1, nSurf
      currentTDMOOverlap(:,iNewSurf) = theta(iNewSurf)*currentTDMOOverlap(:,iNewSurf)
    end do

  end subroutine MOPhaseFix

  subroutine phaseFix(nSurf, currentEigVec, currentTDMOOverlap,&
    & tFindMatchSurf, tPrintTDSameMOOverlap, fdPrintTDSameMOOverlap)
    use omp_lib
    implicit none
    !Interface variables
    integer, intent(in) :: nSurf
    real(dp), intent(in) :: currentTDMOOverlap(:,:) 
    real(dp), intent(inout) :: currentEigVec(:,:)
    logical, intent(in) :: tPrintTDSameMOOverlap
    logical, intent(in) :: tFindMatchSurf
    integer, intent(in), optional :: fdPrintTDSameMOOverlap
    ...
    !Local variables
    integer :: iNewSurf, iAO, jAO
    real(dp) :: modTheta(nSurf), theta(nSurf)
    real(dp) :: maxOvlpValue(nSurf) 
    integer :: maxOvlpSurf(nSurf) 
    real(dp), parameter :: maxTol = 0.707
    character(20) :: my_fmt4, my_fmt5
    logical :: match(nSurf)
    ...
    if(tFindMatchSurf) then ! Fix the phase using Matched state's Phase
      do iNewSurf = 1, nSurf
        maxOvlpValue(iNewSurf) = maxval(abs(currentTDMOOverlap(:,iNewSurf)),dim=1)
        maxOvlpSurf(iNewSurf) = maxloc(abs(currentTDMOOverlap(:,iNewSurf)),dim=1)
        if (maxOvlpValue(iNewSurf) > maxTol) then
          theta(iNewSurf) = currentTDMOOverlap(maxOvlpSurf(iNewSurf),iNewSurf)
          match(iNewSurf) = .true.
        else ! Fix the phase using same state's Phase
          theta(iNewSurf) = currentTDMOOverlap(iNewSurf,iNewSurf) 
        end if
      enddo
    else ! Fix the phase using same state's Phase
      do iNewSurf = 1, nSurf
    ...
    if(tPrintTDSameMOOverlap) then
      write(fdPrintTDSameMOOverlap,my_fmt4) theta
      if(tFindMatchSurf) then ! Fix the phase using Matched state's Phase
        write(fdPrintTDSameMOOverlap,my_fmt5) maxOvlpSurf
        write(fdPrintTDSameMOOverlap,*) match
      end if
    end if
    ...
    do iNewSurf = 1, nSurf
      currentEigVec(:,iNewSurf) = theta(iNewSurf)*currentEigVec(:,iNewSurf)
    end do

    !! do iNewSurf = 1, nSurf
    !!   !$omp parallel do
    !!     do iAO= 1, sqrHamSize
    !!       temp1(iAO,iNewSurf) =  SUM(currentEigVec(:,iNewSurf)*oldOverlap(iAO,:))
    !!     end do
    !!   !$omp end parallel do
    !! end do

    !! do iNewSurf = 1, nSurf
    !!   theta(iNewSurf) = SUM(oldEigVec(:,iNewSurf)*temp1(:,iNewSurf))
    !! end do

  end subroutine phaseFix 

  subroutine pickActiveSurf(nSurf, beginSurfId, endSurfId, adiaPop,&
    & accumulator, dice, iActiveSurf)
    implicit none
    !Interface variables
    integer, intent(in) :: nSurf, beginSurfId, endSurfId
    real(dp), intent(in) :: adiaPop(:)
    integer, intent(out) :: iActiveSurf 
    real(dp), intent(out) :: accumulator, dice 

    ! Local Variables
    integer :: iSurf
    real(dp) :: tempRand 
    
    ! Here we are omitting random numbers < 0.05. If we want to change
    ! this behavior, then we need to remove the below loop and just use 
    ! call random_number(dice)
    do
      call random_number(tempRand)
      if(tempRand > 0.05) then
        exit
      else
        cycle
      end if
    ...
    do iSurf = 1, nSurf
      if (adiaPop(iSurf) > 0) then
        accumulator = accumulator + adiaPop(iSurf)
        if(accumulator > dice) then
          iActiveSurf = iSurf
          exit
        end if
      end if
    end do
  end subroutine pickActiveSurf

  subroutine linearInterpolation(timeStep, nIntPolSteps, nSurf,&
    & beginSurfId, endSurfId, oldBq, oldEigVal, oldTDMOOverlap, oldDervCplng,&
    & currentBq, currentEigVal, currentTDMOOverlap,currentDervCplng,&
    & iFSSHMethod, iBqPropagScheme, hopProbability, tWriteIntPol,&
    & fdIntpolEigVal, fdIntPolDervCplng)
    ...
    !Interface variables short-time
    integer, intent(in) :: iFSSHMethod, iBqPropagScheme
    integer, intent(in) :: nIntPolSteps
    integer, intent(in) :: nSurf, beginSurfId, endSurfId
    real(dp), intent(in) :: timeStep
    real(dp), intent(in) :: oldEigVal(:), currentEigVal(:) !! E(q,t-1), E(q,t) 
    real(dp), intent(in) :: oldDervCplng(:,:), currentDervCplng(:,:) 
    real(dp), intent(in) :: oldTDMOOverlap(:,:), currentTDMOOverlap(:,:) 
    complex(dp), intent(in) :: oldBq(:) ! B(q,t-1) 
    logical, intent(in) :: tWriteIntPol
    integer, intent(in), optional :: fdIntpolEigVal, fdIntPolDervCplng

    complex(dp), intent(out) :: currentBq(:) ! B(q,t) 
    real(dp), intent(out) :: hopProbability(:,:)

    !Local variables for interpolation
    integer     :: iIntPolStep
    real(dp)    :: tempOldEigVal(nSurf), tempCurrentEigVal(nSurf), diffEigVal(nSurf)
    real(dp)    :: tempOldDervCplng(nSurf,nSurf), tempCurrentDervCplng(nSurf,nSurf)
    real(dp)    :: tempOldTDMOOverlap(nSurf,nSurf), tempCurrentTDMOOverlap(nSurf,nSurf)
    real(dp)    :: diffDervCplng(nSurf,nSurf)
    real(dp)    :: diffTDMOOverlap(nSurf,nSurf)
    complex(dp) :: tempOldBq(nSurf), tempCurrentBq(nSurf)

    !Local variables for Hopping
    integer :: iOldSurf, iNewSurf, kActiveSurf, iSurf, kSurf
    real(dp) :: a_ki(nSurf,nSurf), b_ik(nSurf,nSurf), time_now
    CHARACTER(20) :: my_fmt1,my_fmt2

    write(my_fmt1,'(a, i4, a)') '(f12.6,', nSurf,'f12.6)'
    write(my_fmt2,'(a, i4, a)') '(f12.6,', nSurf*nSurf,'f12.6)'

    ! Interpolation Scheme Starts
    tempOldEigVal = oldEigVal
    diffEigVal = currentEigVal - oldEigVal
 
    tempOldDervCplng = oldDervCplng
    diffDervCplng = currentDervCplng - oldDervCplng

    tempOldTDMOOverlap = oldTDMOOverlap
    diffTDMOOverlap = currentTDMOOverlap - oldTDMOOverlap
    
    tempOldBq = oldBq
    hopProbability = 0.0d0
    do iIntPolStep = 1, nIntPolSteps+1

      time_now = time_now + (timeStep/real(nIntPolSteps+1))

      tempCurrentEigVal = oldEigVal + (diffEigVal * &
                        & real(iIntPolStep)/real(nIntPolSteps+1))
     
      if(tWriteIntPol)then
        write(fdIntpolEigVal,my_fmt1)time_now, tempCurrentEigVal
      end if

      if (iBqPropagScheme == 1) then
        tempCurrentTDMOOverlap = oldTDMOOverlap + (diffTDMOOverlap * &
                           & real(iIntPolStep)/real(nIntPolSteps+1)) 
      
        call shortTimeBqPropagation((timeStep/real(nIntPolSteps+1)), nSurf, &
          & tempOldBq, tempOldEigVal, tempCurrentEigVal, &
          & tempcurrentTDMOOverlap, tempCurrentBq)
    
        tempOldTDMOOverlap = tempCurrentTDMOOverlap

      else if (iBqPropagScheme == 2) then
        tempCurrentDervCplng = oldDervCplng + (diffDervCplng * &
                           & real(iIntPolStep)/real(nIntPolSteps+1)) 
      
        if(tWriteIntPol)then
          write(fdIntPolDervCplng,my_fmt2)time_now, tempCurrentDervCplng
        end if

        call RK4BqPropagation((timeStep/real(nIntPolSteps+1)), nSurf,&
          & tempOldBq, tempOldEigVal, tempOldDervCplng, tempCurrentBq,&
          & tempCurrentEigVal, tempCurrentDervCplng)

        tempOldDervCplng = tempCurrentDervCplng
      end if

      ! As per the Hammes-Schiffer and Tully's J.Chem.Phys 1994 paper
      if (iFSSHMethod == 1) then
        do kActiveSurf = 1,nSurf
          b_ik(:,kActiveSurf) = -2.0d0 * REAL(conjg(tempCurrentBq(:))*&
             & (tempCurrentBq(kActiveSurf)) * tempCurrentDervCplng(:,kActiveSurf))
          hopProbability(kActiveSurf,:) = hopProbability(kActiveSurf,:) + b_ik(:,kActiveSurf)
        end do
      end if

      tempOldEigVal = tempCurrentEigVal
      tempOldBq = tempCurrentBq
    end do
    currentBq = tempCurrentBq 
    ! Interpolation Scheme Ends

    ! As per the Hammes-Schiffer and Tully's J.Chem.Phys 1994 paper
    if (iFSSHMethod == 1) then
      do kActiveSurf = 1,nSurf
        a_ki(kActiveSurf,:) = conjg(oldBq(:))*(oldBq(kActiveSurf)) ! a_ki = (C_k)*conjg(C_i)
        hopProbability(kActiveSurf,:) = (hopProbability(kActiveSurf,:)/&
          & a_ki(kActiveSurf,kActiveSurf))*((timeStep)/real(nIntPolSteps+1))
      end do

    ! As per Tully's J.Chem.Phys 1990 paper
    else if (iFSSHMethod == 2) then
      do kActiveSurf = 1,nSurf
        a_ki(kActiveSurf,:) = conjg(currentBq(:))*(currentBq(kActiveSurf))
        b_ik(:,kActiveSurf) = -2.0d0 * REAL(conjg(currentBq(:))*&
           & (currentBq(kActiveSurf)) * currentDervCplng(:,kActiveSurf))
        hopProbability(kActiveSurf,:) = b_ik(:,kActiveSurf)*timeStep/a_ki(kActiveSurf,kActiveSurf)
      end do
    end if

    ! scaling with Boltzmann weights
    do iSurf = 1, nSurf
      do kActiveSurf = 1,nSurf
        if(currentEigVal(iSurf) > currentEigVal(kActiveSurf)) then
          ! 25.7 meV is the value of (k_B)*T at 25 centigrade
          hopProbability(kActiveSurf,iSurf) = hopProbability(kActiveSurf,iSurf)*&
            & exp((currentEigVal(kActiveSurf) - currentEigVal(iSurf))/0.0257d0)
        end if
      end do   
    end do   
  ...
  subroutine FSSHmethod(nSurf, hopProbability, iActiveSurf, iNextSurf,&
    & minRandNum, fdRandNumSeq)
    
    implicit none
    !Interface variables
    real(dp), intent(in) :: minRandNum
    real(dp), intent(in) :: hopProbability(:,:)
    integer, intent(in)  :: nSurf
    integer, intent(in)  :: iActiveSurf
    integer, intent(inout) :: iNextSurf
    integer, intent(in), optional  :: fdRandNumSeq
    
    !Local variables
    integer :: iOldSurf, iNewSurf, iSurf, kSurf 
    real(dp) :: tempRand, dice, accumulator
    logical  :: tDownWardJump

    ! Routine is written as per the Hammes-Schiffer and Tully's J.Chem.Phys 1994 paper
    do
      call random_number(tempRand)
      if(tempRand > minRandNum) then
        exit
      else
        cycle
      end if
    ...
    if(present(fdRandNumSeq)) then
      write(fdRandNumSeq,'(f10.6)') tempRand
    end if
     
    ! This implementation has an inherent bias towards jumping
    ! down (as we are accumulating from 1st state). However,
    ! this seems to me as natural as generally one would like 
    ! to stay lower potential energy surface than the upper ones.

    ! downward transitions (if any)
    tDownWardJump = .false.
    do kSurf = 1, iNextSurf-1
      if (hopProbability(iActiveSurf,kSurf) > 0) then
        accumulator = accumulator + hopProbability(iActiveSurf,kSurf)
        if(accumulator > dice) then
          iNextSurf = kSurf ! We have jumped to kSurf
          tDownWardJump = .true.
          exit
        end if
      end if
    ...
    if (.not. tDownWardJump) then
      do kSurf = iNextSurf+1, nSurf
        if (hopProbability(iActiveSurf,kSurf) > 0)then
          accumulator = accumulator + hopProbability(iActiveSurf,kSurf)
          if(accumulator > dice)then
            iNextSurf = kSurf 
  ...
  end subroutine FSSHmethod
        
  subroutine subotnikEstimators(sqrHamSize, nSurf, currentBq, currentEigVec,&
    & currentOverlap, beginAOId, endAOId, estimator2, totEstimator2,&
    & FSSHAdiaPop, estimator1, totEstimator1, estimator3, totEstimator3)
    ...
    !Interface variables
    integer, intent(in) :: sqrHamSize, nSurf
    integer, intent(in) :: beginAOId(:), endAOId(:)
    real(dp), intent(in) :: currentOverlap(:,:), currentEigVec(:,:)
    complex(dp), intent(in) :: currentBq(:)
    real(dp), intent(out) :: estimator2(:)
    real(dp), intent(out) :: totEstimator2
    ! Optional arguments need explicit interface blocks if they are
    ! NOT "contain"ed in the main program. Remember this
    real(dp), intent(in), optional :: FSSHAdiaPop(:)
    real(dp), intent(out), optional :: estimator1(:), estimator3(:)
    real(dp), intent(out), optional :: totEstimator1, totEstimator3

    !Local variables
    integer :: iLoop, iSurf, jSurf, iAO, jAO

    do iFrag=1,nFrags
      call fragmentChargePop(sqrHamSize, nSurf,&
        & currentBq, currentEigVec, currentOverlap, beginAOId(iFrag),&
        & endAOId(iFrag), estimator2(iFrag), FSSHAdiaPop,&
        & estimator1(iFrag), estimator3(iFrag))
    end do

    call fragmentChargePop(sqrHamSize, nSurf,&
      & currentBq, currentEigVec, currentOverlap, 1, sqrHamSize,&
      & totEstimator2, FSSHAdiaPop, totEstimator1, totEstimator3)

  end subroutine subotnikEstimators

  subroutine fragmentChargePop(sqrHamSize, nSurf,&
    & currentBq, currentEigVec, currentOverlap, iBeginAOId,&
    & iEndAOId, est2, FSSHAdiaPop, est1, est3)

    implicit none
    !Interface variables
    integer, intent(in) :: sqrHamSize, nSurf
    integer, intent(in) :: iBeginAOId, iEndAOId 
    real(dp), intent(in) :: currentOverlap(:,:), currentEigVec(:,:)
    complex(dp), intent(in) :: currentBq(:)
    real(dp), intent(out) :: est2
    ! Optional arguments need explicit interface blocks if they are
    ! NOT "contain"ed in the main program. Remember this
    real(dp), intent(in), optional :: FSSHAdiaPop(:)
    real(dp), intent(out), optional :: est1, est3 

    !Local variables
    integer  :: iSurf, jSurf, iAO, jAO
    real(dp) :: rho(nSurf,nSurf), est2Diag, est2OffDiag

    !rho is saved and used for efficiency purposes only.
    do iSurf = 1, nSurf
      do jSurf = 1, nSurf
        rho(iSurf,jSurf) = real(currentBq(iSurf)*conjg(currentBq(jSurf)))
      end do
    end do

    est2Diag=0.0d0
    do iSurf = 1, nSurf
      do jAO = iBeginAOId, iEndAOId
        do iAO = 1, sqrHamSize
          est2Diag = est2Diag + (currentEigVec(iAO,iSurf)*rho(iSurf, iSurf)*&
                         & currentEigVec(jAO,iSurf)) * currentOverlap(iAO,jAO)
        end do
      end do
    end do
    ...
    do iSurf = 1, nSurf
      do jSurf = iSurf+1, nSurf
        do jAO = iBeginAOId, iEndAOId
          do iAO = 1, sqrHamSize
            est2OffDiag = est2OffDiag + (currentEigVec(iAO,iSurf)*rho(iSurf,jSurf)* &
                              & currentEigVec(jAO,jSurf)) * currentOverlap(iAO,jAO)
          end do
        end do
      end do
    end do

    est2OffDiag = (2.0d0*est2OffDiag) ! 2.0d0 is for jSurf going from iSurf+1 to nSurf
    ...
    est2=0.0d0
    est2 = est2Diag + est2OffDiag

    if(present(FSSHAdiaPop)) then
      ...
      do iSurf = 1, nSurf
        do jAO=iBeginAOId, iEndAOId
          do iAO=1, sqrHamSize
            est1 = est1 + (currentEigVec(iAO,iSurf)*FSSHAdiaPop(iSurf)*&
                       & currentEigVec(jAO,iSurf))* currentOverlap(iAO,jAO)
          end do
        end do
      end do
      ...
      est3=0.0d0
      est3 = est1 + est2OffDiag
     
   end if

  end subroutine fragmentChargePop
