...
contains
  ...
  subroutine writeRealEigvecs(fdEigvec, runId, nAtom, nSpin, neighlist, &
      ...
      &over, HSqrReal, SSqrReal, mdStepNum, storeEigvecs, fileName)
    ...
    character(len=*), intent(in), optional :: mdStepNum
    ...
    else if (present(mdStepNum)) then
      write (tmpStr, "(A,A,A)") "./eigvecs_dir/eigvecs_",trim(mdStepNum), ".bin"
      open(fdEigvec, file=tmpStr, action="write", status="replace", &
          &position="rewind", form="unformatted")
    else
    ...
    if (EigVecsAsTxt_) then
      ...
      if (present(fileName)) then
        ...
        write (tmpStr, "(A,A,A)") "./eigvecs_dir/eigvecs_",trim(mdStepNum), ".dat"
        open(fdEigvec, file=tmpStr, action="write", status="replace", &
            &position="rewind")
      else
      ...
      close(fdEigvec)
      ! cat followed by sed is found to be faster than direct sed -i
      call execute_command_line("cat ./eigvecs_dir/eigvecs_" // adjustl(trim(mdStepNum)) // ".dat &
           & | sed '/Eige/d' | sed '1d' | sed '/^\s*$/d' | sed '/^$/d'  > temp.out" )
      call execute_command_line("cat temp.out | awk '{printf" // '"%12.6f\n"' //", $(NF-1)}' &
           & > ./eigvecs_dir/eigvecs_" // adjustl(trim(mdStepNum)) // ".dat")
  ...
  subroutine writeCplxEigvecs(fdEigvec, runId, nAtom, nSpin, neighlist, &
      ...
      &species, speciesName, over, kpoint, HSqrCplx, SSqrCplx, mdStepNum, storeEigvecs, &
      & fileName)
    integer, intent(in) :: fdEigvec, runId, nAtom, nSpin
    type(TNeighborList), intent(in) :: neighlist
    integer, intent(in) :: nNeighbor(:)
