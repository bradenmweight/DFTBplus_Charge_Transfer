...
204 format(A5,7F16.8)
  ...
  subroutine writeSparseAsSquare_real(fname, sparse, iNeighbor, nNeighbor, &
    ...
    integer, intent(in), optional :: fdOverlap
    ...
    if (present(fdOverlap))then
      fd = fdOverlap 
      if(present(tWriteOvlpBin)) then
        if(tWriteOvlpBin) then
          open(fd, file=fname, form="unformatted", status="replace")
