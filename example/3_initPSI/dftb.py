...
def getPsi(ith,xyzFilename,filename = "eigenvec.out"):
    ...
    #out = open("eigen.txt","w+")
    #out.writelines(fob[start:end])
    #out.close()
    if (end != False) :
...
def findHomo(detailedFile = "detailed.out"):
    ...
    if (totalElectron%2) != 0:
        print( "NELECT = ", totalElectron )
        print( "NELECT (Attempted fix):", np.round(totalElectron), "-->", int(np.round(totalElectron)) )
        print( "NHOMO  (Attempted fix):", np.round(totalElectron/2), "-->", int(np.round(totalElectron)) // 2 )
        if ( abs(np.round(totalElectron) - totalElectron) > 0.1 ):
            raise Exception('Number of electrons is odd or wrong !')
        else: # Slightly off from NELECT, e.g., 21.999999 or 22.000001
            return int(np.round(totalElectron)) // 2
    return totalElectron // 2
